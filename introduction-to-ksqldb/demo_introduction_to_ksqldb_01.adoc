...
Chris Harmon <charmon@confluent.io>
:toc:
...
[source,bash]
----
git clone https://github.com/charmon79/confluent-demos.git
...
[source,sql]
----
SELECT TIMESTAMPTOSTRING(ROWTIME,'yyyy-MM-dd HH:mm:ss','US/Central') AS EVENT_TS, 
       PERSON,
       LOCATION 
  FROM MOVEMENTS
  EMIT CHANGES;
...
[source,sql]
----
INSERT INTO MOVEMENTS VALUES ('Chris', 'Omaha');
INSERT INTO MOVEMENTS VALUES ('Chris', 'Kansas City');
INSERT INTO MOVEMENTS VALUES ('Chris', 'Des Moines');
----
...
[source,sql]
----
INSERT INTO MOVEMENTS VALUES ('Chris', 'St. Louis');
INSERT INTO MOVEMENTS VALUES ('Chris', 'Omaha');
INSERT INTO MOVEMENTS VALUES ('Chris', 'Wichita');
INSERT INTO MOVEMENTS VALUES ('Chris', 'Omaha');
----
...
CREATE STREAM OMAHA_USERS WITH (KAFKA_TOPIC='omaha-users') 
    AS SELECT * FROM MOVEMENTS WHERE LCASE(LOCATION)='omaha' EMIT CHANGES;
----
...
[source,sql]
----
PRINT 'omaha-users' FROM BEGINNING;
----
...
SELECT TIMESTAMPTOSTRING(ROWTIME,'yyyy-MM-dd HH:mm:ss','US/Central') AS EVENT_TS, PERSON, LOCATION 
...
----
INSERT INTO MOVEMENTS VALUES ('Chris', 'Omaha');
INSERT INTO MOVEMENTS VALUES ('Chris', 'Lincoln');
----
...
docker exec -t ksqldb curl -s -X "POST" "http://localhost:8088/query" \
     ...
     -d '{"ksql":"SELECT PERSON, LATEST_LOCATION, LOCATION_CHANGES, UNIQUE_LOCATIONS FROM PERSON_STATS WHERE PERSON='\''Chris'\'';"}'|jq '.[].row'
----
...
[source,sql]
----
INSERT INTO MOVEMENTS (ROWTIME, PERSON, LOCATION) VALUES (STRINGTOTIMESTAMP('2020-02-17T15:22:00Z','yyyy-MM-dd''T''HH:mm:ssX'), 'Chris', 'Kansas City');
INSERT INTO MOVEMENTS (ROWTIME, PERSON, LOCATION) VALUES (STRINGTOTIMESTAMP('2020-02-17T16:22:00Z','yyyy-MM-dd''T''HH:mm:ssX'), 'Chris', 'Springfield');
----
...
docker exec -t ksqldb curl -s -X "POST" "http://localhost:8088/ksql" \
     -H "Content-Type: application/vnd.ksql.v1+json; charset=utf-8" \
     -d '{
            "ksql":"CREATE STREAM Lincoln AS SELECT * FROM MOVEMENTS WHERE LCASE(LOCATION)='\''Lincoln'\'';",
            "streamsProperties": {
